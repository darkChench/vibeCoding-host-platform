...
- [ ] 查询条件表单：开始时间/结束时间/点位多选/导出格式（CSV/Excel）
- [ ] 点位多选下拉，选项来自 store.params 的采样参数
- [ ] 查询按钮：校验时间范围（开始<结束）+ 选中点位非空 → loading → 曲线渲染
- [ ] 趋势曲线（QtCharts）按选中点位生成，颜色按调色板分配
- [ ] 导出按钮：pandas 写 CSV 到 ./save，loading 后成功提示
- [ ] 未选点位时查询/导出拦截
- [ ] 空查询结果显示空态
