...
- [x] metric 卡数量 = store.params 的采样参数数，动态跟随增删
- [x] metric 值每 1s 走协议层 read_param 刷新，按 decimals 格式化
- [x] 趋势曲线（QtCharts）按采样参数动态生成，颜色按调色板分配
- [x] 曲线 chip 点击显隐对应曲线（颜色稳定，隐藏不影响其他曲线色）
- [x] 通信失败时 metric 显示 `--`
- [x] 未连接时显示空态"串口未连接"
- [x] 无采样参数时显示空态"暂无采样参数"
- [x] 离开页面时停止刷新定时器
